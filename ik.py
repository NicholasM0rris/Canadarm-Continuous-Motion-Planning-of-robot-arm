    ...
    for i in range(len(robot_config) - 1):
        ...
        for step_number in range(n_steps):
            ...
            if num_links == 3:
                ...
                print(ci[2:])
                print(ee1)
                a_configuration = make_robot_config_from_ee1(x=ee1[0], y=ee1[1], angles=initial_angles, lengths=ci[2:])
                ...
                x = ci[0] - f_x
                y = ci[1] - f_y
                print('the config', robot_configuration[i], robot_configuration[i + 1])
...
def inverse_kinematics(a1, a2, x, y):
    """
    Finds the inverse kinematics for 2 link robot arm
    :param a1: length arm 1
    :param a2: length arm 2
    :param x: desired x coord
    :param y: desired y coord
    :return: Angles elbow down and elbow up angles
    """
    # Elbow down
    print(a1, a2, x, y)
    ...
    # Elbow up
    q2u = (-1) * np.arccos((x ** 2 + y ** 2 - a1 ** 2 - a2 ** 2) / (2 * a1 * a2))
    q1u = np.arctan(y / x) + np.arctan((a2 * np.sin(q2u)) / (a1 + a2 * np.cos(q2u)))
    # print(q1u, q2u)
    # print((x ** 2 + y ** 2 - a1 ** 2 - a2 ** 2) / (2 * a1 * a2))
    # print(x ** 2 + y ** 2)
    # print(a1 ** 2 - a2 ** 2)
    # print(2 * a1 * a2)
    return [[q1, q2], [q1u, q2u]]
...
def write_robot_config_list_to_file(filename, robot_config_list, num_links):
    ...
    for rc in range(len(robot_config_list)):
        ...
        for i in range(num_links):
            # print(robot_config_list[rc])
            lengths.append(robot_config_list[rc][i + 2 + num_links])
            angles.append(robot_config_list[rc][2 + i])
        '''
...
def main():
    ...
    prm = PRM(1, 0, 1, 0, 400, problem_spec.obstacles, problem_spec.grapple_points, problem_spec.goal.get_ee2()[0],
