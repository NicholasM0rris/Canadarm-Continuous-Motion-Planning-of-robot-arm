...
class PRM:
    ...
    def interpolate_collision_check(self, node1, node2, num_links):
        ...
        n_steps = math.ceil(max_diff / 0.1)
        ...
        for step_number in range(n_steps):
            ...
            if not test_obstacle_collision(config, self.spec, self.obstacle) or not self.test_self_collision(config, self.spec) or not self.inside_world_point(ee2):
                return True
...
def main():
    ...
    prm = PRM(1, 0, 1, 0, 300, problem_spec.obstacles, problem_spec.grapple_points, problem_spec.goal.get_ee2()[0],
